    ...
    sample_interval(20)
{
    ...
    private_nh.param(std::string("use_two_motor_encoder_only"),use_two_motor_encoder_only,false);
...
void Model::cmdVelSubCB(const geometry_msgs::TwistConstPtr&msg){
    tarV=msg->linear.x;
    int dir=(tarW>0)?1:-1;
    tarW=(fabs(msg->angular.z)>0.5)?(0.5*dir):(msg->angular.z);
    robotMove();
}
...
void Model::PosnVelMotor2Robot(){
    ...
    double dL,dR;
//20181221
    if(use_two_motor_encoder_only){	
       dL=-(double)deltaMP[1]/RatioCountsPerMeter;
       dR=(double)deltaMP[0]/RatioCountsPerMeter;
    }else{
